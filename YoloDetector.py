...
def draw_box_string(img, string, org, thickness):
    ...
    font = ImageFont.truetype(font="chinese.simhei.ttf", size=thickness * 5, encoding="unic")
    draw.text(xy=(org[0], org[1] - 25), text=string, fill=(0, 0, 0), font=font)
    img = np.asarray(img)
    cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    return img


def plot_one_box(x, img, color=None, label=None, line_thickness=1, font_scale=0.4, font_thickness=1):
    # Plots one bounding box on image img

    # Draw the rectangle with specified line thickness
    ...
    # Label text
    if label.split(" ")[0] in ["person", "observer", "chair", "laptop", "mouse", "fps", "keyboard", "finger print", "cell phone", "cell", "book", "paper"]: #"backpack", "handbag", "cup", "fork", "knife", "spoon","bowl",
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, font_scale, [0, 0, 0], thickness=font_thickness, lineType=cv2.LINE_AA)
    else:
        img = draw_box_string(img, label, (c1[0], c1[1] - 2), font_thickness)
...
class YOLOV8_Detector:
    ...
    def Detect(self, img0, objects_to_detect_id_name,
        ...
        for value in df.values:
            ...
            object_confidence = round(value[-1], 2)
            ...
            if current_person > 1:
                ...
                if objects_to_detect_id_name[0] == "person":
                    ...
                    img0 = plot_one_box([x1, y1, x2, y2],
                                        ...
                                        label="observer: " + str(current_person), # + " " + str(object_confidence),
        ...
        for i, det in enumerate(boxes_list):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    object_confidence = round(conf, 2)
